...
# Overview
RedEye is a data processing pipeline for use in the assembly of cross sectional survey (large scale and otherwise) sampling pools, designed with accessibility for users with limited programming expertise in mind. By leveraging multi-threading, it is able to rapidly extracted author linked metadata (ie; author emails, institutions, etc...) from a provided list of PMIDs or an OVID output file. Via these methods, the contact information of tens of thousands of authors can be extracted within minutes. To learn how to use RedEye, please see the attached [tutorial](RedEye_Tutorial.md)
